...
class DQN(nn.Module):
    ...
    def __init__(self, observation_space: spaces.Box, action_space: spaces.Discrete):
        ...
        self.fc1 = nn.Linear(input_size, 512) #flattening
        self.fc2 = nn.Linear(512, action_space.n) # 512 in, number of actions out, bc the number of actions is the number of classes.

        # allows our code to be run on the GPU
        self.device = torch.device("cuda")
        self.to(self.device)

    def get_input_size(self, input_dims):
        # function that does a forward pass of the code so that we can find out the input size
    ...
    def forward(self, x):
        # implements a forward pass through neural network
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = x.view(x.size()[0], -1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x) # this is our output layer, so we dont need to use an activation function
