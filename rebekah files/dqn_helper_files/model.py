...
class DQN(nn.Module):
    ...
    def __init__(self, observation_space: spaces.Box, action_space: spaces.Discrete):
        ...
        self.fc2 = nn.Linear(512, action_space.n) # 1024 in, number of actions out, bc the number of actions is the number of classes.
    ...
    def forward(self, x):
        ...
        x = x.view(x.size()[0], -1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x) # our output layer, so we dont need to use an activation function
