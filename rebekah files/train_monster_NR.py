...
if __name__ == '__main__':
    ...
    hyper_params = {
        ...
        "env-name": "MiniHack-Room-Monster-15x15-v0",  # name of the game
        "replay-buffer-size": int(5e3),  # replay buffer size
        "learning-rate": 1e-3,  # learning rate for Adam optimizer
        "discount-factor": 0.99,  # discount factor
        "num-steps": 200000,  # total number of steps to run the environment for
  ...
  #  env = RenderRGB(env, "pixel_crop")
    ...
    torch.save(agent.Q_current.state_dict(), "dqn_monster_NR.pth")
    ...
    plt.savefig("dqn_monster_NR_re_graph.png")
    #plt.show()
    ...
    plt.xlabel("Episodes")
    plt.ylabel("Loss")
    plt.savefig("dqn_monster_NR_loss_graph.png")
    #plt.show()
    np.save("monster_NR_reward_array.npy",episode_rewards)
    np.save("monster_NR_loss_array.npy",episode_loss)
    agent.save("monster_NR_dqn")
